...
// Trigger Quick Scan and Store Results
  ...
  try {
    ...
    exec(
      ...
      (error, stdout, stderr) => {
        ...
        // Parse the output from the Python script (assuming it's in JSON format)
        try {
          ...
          scan.save()
            .then(async () => {
              ...
              res.status(200).json({ message: "Scan completed and results saved", scanResults });
            })
            .catch((err) => {
              res.status(500).json({ message: "Error saving scan results", err });
            });
        } catch (jsonError) {
          return res.status(500).json({ message: "Error parsing Python script output", jsonError });
        }
      }
    );
  } catch (error) {
